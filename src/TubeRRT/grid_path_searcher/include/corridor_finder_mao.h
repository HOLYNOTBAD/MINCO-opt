...
class tubeRrtStar
{
	private:
		...
		double safety_margin, max_radius, search_margin, sample_range, max_volume;
		...
		// coefficient of cost for path length and intersected volume
		double rho_d = 1, rho_v= 0.1;
  ...
  public:
		...
		inline NodePtr tubeSteer( Eigen::Vector3d & pt_sample, NodePtr node_nearst_ptr );
		...
		inline double intersectVolume(NodePtr node_1, NodePtr node_2);

		/* data return */
		pair<Eigen::MatrixXd, Eigen::VectorXd> getPath()
		{
			// 把局部变量 Path 和 Radius 打包成一个 std::pair 并作为函数返回值返回
