#include "corridor_finder_mao.h"
#include <algorithm>

using namespace Eigen;
using namespace std;

tubeRrtStar::tubeRrtStar( ){ 
      cach_size  = 100;
}

tubeRrtStar::~tubeRrtStar(){ }

void tubeRrtStar::setParam( double safety_margin_, double search_margin_, double max_radius_, double sample_range_ )
{     
    safety_margin = safety_margin_;
    search_margin = search_margin_;
    max_radius    = max_radius_;
    sample_range  = sample_range_;
    max_volume = 4 / 3 * M_PI * pow(max_radius, 3);
}

void tubeRrtStar::reset()
{     
    treeDestruct();
    ...
    best_end_ptr = new tubeNode();
    root_node    = new tubeNode();
...
void tubeRrtStar::setStartPt( Vector3d startPt, Vector3d endPt)
{
    start_pt = startPt;
    end_pt   = endPt; 
...
void tubeRrtStar::setPt( Vector3d startPt, Vector3d endPt, double xl, double xh, double yl, double yh, double zl, double zh,
...
void tubeRrtStar::setInput(pcl::PointCloud<pcl::PointXYZ> cloud_in)
{     
    kdtreeForMap.setInputCloud( cloud_in.makeShared() );    
}

inline double tubeRrtStar::getDis(const NodePtr node1, const NodePtr node2){
      return sqrt(pow(node1->coord(0) - node2->coord(0), 2) + pow(node1->coord(1) - node2->coord(1), 2) + pow(node1->coord(2) - node2->coord(2), 2) );
}

inline double tubeRrtStar::getDis(const NodePtr node1, const Vector3d & pt){
      return sqrt(pow(node1->coord(0) - pt(0), 2) + pow(node1->coord(1) - pt(1), 2) + pow(node1->coord(2) - pt(2), 2) );
}

inline double tubeRrtStar::getDis(const Vector3d & p1, const Vector3d & p2){
      return sqrt(pow(p1(0) - p2(0), 2) + pow(p1(1) - p2(1), 2) + pow(p1(2) - p2(2), 2) );
}

inline double tubeRrtStar::radiusSearch( Vector3d & search_Pt)
{     
...
void tubeRrtStar::clearBranchW(NodePtr node_delete) // Weak branch cut: if a child of a node is on the current best path, keep the child
{     
...
void tubeRrtStar::clearBranchS(NodePtr node_delete) // Strong branch cut: no matter how, cut all nodes in this branch
{     
    for( auto nodeptr: node_delete->nxtNode_ptr ){
        if( nodeptr->valid)
            invalidSet.push_back(nodeptr);
...
void tubeRrtStar::treePrune(NodePtr newPtr)
{
    NodePtr ptr = newPtr;
    // if (ptr->g + ptr->f > best_distance)
    // { // delete it and all its branches
    //     ptr->valid = false;
    //     invalidSet.push_back(ptr);
    //     clearBranchS(ptr);
    // }
    if (ptr->cost > best_distance)
    { // delete it and all its branches
        ptr->valid = false;
        invalidSet.push_back(ptr);
        clearBranchS(ptr);
...
void tubeRrtStar::removeInvalid()
{     
...
void tubeRrtStar::resetRoot(Vector3d & target_coord)
{     
...
void tubeRrtStar::solutionUpdate(double cost_reduction, Vector3d target)
{
    for(auto nodeptr: NodeList){
...
void tubeRrtStar::updateHeuristicRegion(NodePtr update_end_node)
{   
...
inline Vector3d tubeRrtStar::genSample()
{
    Vector3d pt;
...
inline NodePtr tubeRrtStar::tubeSteer( Eigen::Vector3d & pt_sample, NodePtr node_nearst_ptr )
{
    double dis = getDis(node_nearst_ptr, pt_sample);
    double radius_new = radiusSearch(pt_sample);
    double radius_nearest = node_nearst_ptr->radius;
    Vector3d center_new = pt_sample;

    double maxRadius = std::max(radius_nearest, radius_new);
    while (dis > radius_nearest+radius_new)
    {
        double steer_dis = maxRadius / dis;
        center_new(0) = node_nearst_ptr->coord(0) + (center_new(0) - node_nearst_ptr->coord(0)) * steer_dis;
        center_new(1) = node_nearst_ptr->coord(1) + (center_new(1) - node_nearst_ptr->coord(1)) * steer_dis;
        center_new(2) = node_nearst_ptr->coord(2) + (center_new(2) - node_nearst_ptr->coord(2)) * steer_dis;
        radius_new = radiusSearch(center_new);
        maxRadius = std::max(radius_nearest, radius_new);
        dis = getDis(node_nearst_ptr, center_new);
        // std::cout << "maxradius: " << maxRadius << std::endl;
        // std::cout << "radius_nearest: " << radius_nearest << std::endl;
        // std::cout << "radius_new: " << radius_new << std::endl;
        // std::cout << "dis: " << dis << std::endl;
    }

    double h_dis_ = getDis(center_new, end_pt);

    NodePtr node_new_ptr = new tubeNode(center_new, radius_new, inf, h_dis_, 0);

    return node_new_ptr;
}

inline NodePtr tubeRrtStar::genNewNode( Vector3d & pt_sample, NodePtr node_nearst_ptr )
{
    double dis       = getDis(node_nearst_ptr, pt_sample);
    ...
    NodePtr node_new_ptr = new tubeNode( center, radius_, inf, h_dis_ , 0); 
    
    return node_new_ptr;
}

bool tubeRrtStar::checkTrajPtCol(Vector3d & pt)
{     
    if(radiusSearch(pt) < 0.0 ) return true;
    else return false;
}

inline bool tubeRrtStar::checkEnd( NodePtr ptr )
{    
...
inline NodePtr tubeRrtStar::findNearstVertex( Vector3d & pt )
{
...
inline int tubeRrtStar::checkNodeRelation( double dis, NodePtr node_1, NodePtr node_2 )
{
...
inline double tubeRrtStar::intersectVolume(NodePtr node_1, NodePtr node_2)
{
    double interVol = 0;

    double d = getDis(node_1, node_2);
    double r = node_1->radius;
    double R = node_2->radius;

    double cos_alpha = (pow(r, 2) + pow(d, 2) - pow(R, 2)) / (2 * r * d);
    double h2 = r - r * cos_alpha;

    double cos_beta = (pow(R, 2) + pow(d, 2) - pow(r, 2)) / (2 * R * d);
    double h1 = R - R * cos_beta;

    interVol = M_PI * pow(h1, 2) * (R - 1 / 3 * h1) + M_PI * pow(h2, 2) * (r - 1 / 3 * h2);

    return interVol;
}

#if 1
void tubeRrtStar::treeRewire(NodePtr node_new_ptr, NodePtr node_nearst_ptr)
{
    NodePtr newPtr = node_new_ptr;
    NodePtr nearestPtr = node_nearst_ptr;

    float range = newPtr->radius + max_radius; // not completly correct
    ...
    { // Pull all the nodes outside the result data structure from the kd-tree
      // And go through all the nearby vertex, check the relations, to see whether the newPtr should be discarded
        ...
        if (res == -1) // new node have intersection with near nodes
        {
            nearPtrList.push_back(nearPtr);
    ...
    if (isInvalid)
    {
        ...
        { // release all the temporary variables
            nodeptr->rel_id = -2;
            nodeptr->rel_dis = -1.0;
        }
        return;
    }
    double min_dis = nearestPtr->g + getDis(nearestPtr, newPtr); 
    double min_vol = nearestPtr->vol + 1 / (intersectVolume(nearestPtr, newPtr) / max_volume + 1); 
    double min_cost = rho_d * min_dis + rho_v * min_vol;

    newPtr->preNode_ptr = nearestPtr;
    newPtr->g = min_dis;
    newPtr->vol = min_vol;
    newPtr->cost = min_cost;
    ...
    for (auto nodeptr : nearPtrList)
    {
        ...
        double cost_dis = nearPtr->g + dis;
        double cost_vol = nearPtr->vol + 1 / (intersectVolume(nearPtr, newPtr) / max_volume + 1);
        double cost = rho_d * cost_dis + rho_v * cost_vol;

        if (res == -1)
        {
            if (cost < min_cost)
            { // has a shorter path if new_ptr's father is the new node
                min_cost = cost;
    ...
    for (int i = 0; i < int(nearVertex.size()); i++)
    {
        ...
        NodePtr nearPtr = nodeptr;
        if (nearPtr->valid == false)
            continue;

        double dis = getDis(nearPtr, newPtr);
        double cost_dis = dis + newPtr->g;
        double cost_vol = 1 / (intersectVolume(nearPtr, newPtr) / max_volume + 1) + newPtr->vol;
        double cost = rho_d * cost_dis + rho_v * cost_vol;

        if (cost < nearPtr->g)
        { // a near Node changed a father, delete this node from its parent's childer list
            ...
            else
            {
                ...
                for (auto ptr : child)
                {
                    if (ptr->change == true)
                        ...
                        lstNearParent->nxtNode_ptr.push_back(ptr);
                }
...
void tubeRrtStar::recordNode(NodePtr new_node)
{
    NodeList.push_back(new_node);
}

void tubeRrtStar::tracePath()
{   
    // 收集所有实际可行的终点（EndList 里可能包含失效或不再可达的终点）
    vector<NodePtr> feasibleEndList;      
        
    for(auto endPtr: EndList)
    {
        // checkValidEnd: 检查从该终点回溯到根节点的路径上是否存在无效节点或是否已被裁剪
        // checkEnd: 检查该终点自身是否满足终点条件（例如球体包含目标点）
        // endPtr->valid: 终点本身必须仍标记为有效
        if( (checkValidEnd(endPtr) == false) || (checkEnd(endPtr) == false) || (endPtr->valid == false) )
            continue; // 任一条件不满足则跳过该终点
        else
            feasibleEndList.push_back(endPtr); // 否则将其视为可行终点
    }

    // 如果没有任何可行终点，则标记路径不存在并清空相关数据结构
    if( int(feasibleEndList.size()) == 0 )
    {
        //ROS_WARN("[trace path] can't find a feasible path. ");
        path_exist_status = false;     // 标记无路径
        best_distance = inf;           // 重置最优代价
        inform_status = false;         // 取消启发式采样的通知状态
        EndList.clear();               // 清空终点列表
        Path   = Eigen::MatrixXd::Identity(3,3); // 将 Path 置为单位 3x3 矩阵以避免空矩阵
        Radius = Eigen::VectorXd::Zero(3);       // 半径向量置 0
        return;                        // 直接返回
    }

    // 只保留可行终点，替换原有 EndList
    EndList = feasibleEndList;

    // 在所有可行终点中选择“代价”最低的一个作为 best_end_ptr
    // 代价计算：g（从根到该节点的代价） + 节点到终点的距离 + root 到 commit_root 的距离（表示已提交起点偏差）
    ...
    {   
        ...
        if( cost < best_cost )
        {
            best_end_ptr  = nodeptr;   // 记录当前最优终点指针
            best_cost = cost;          // 更新最优代价
            best_distance = best_cost; // 同步更新 best_distance
        }
    }

    // 从 best_end_ptr 开始回溯，构建 PathList（从终点向根逐步回溯）
    ...
    // 将从终点回溯到根的节点按逆序（终点先）压入 PathList
    while( ptr != NULL ) { 
        PathList.push_back( ptr );
        ptr = ptr->preNode_ptr; // 沿 preNode_ptr 回溯
        idx ++;
    }

    // 根据回溯到的节点数量调整 Path 和 Radius 的尺寸
    Path.resize(idx, 3) ;
    Radius.resize( idx );
    idx = 0;
    
    // 将 PathList 中的节点写入 Path 矩阵和 Radius 向量，按从根到终点的顺序存储
    ptr = best_end_ptr;
    idx ++ ;

    while( ptr != NULL ){      
          // Path.rows() - idx 用于把回溯得到的节点逆序写回，使得 Path 第一行为起点，最后一行为终点
          Path.row(Path.rows() - idx) = ptr->coord;                // 写入节点坐标 (x,y,z)
          Radius(Radius.size() - idx) = ptr->radius;               // 写入对应节点半径
          
          ptr = ptr->preNode_ptr;  // 继续向前回溯
          idx ++;
    }
...
inline double tubeRrtStar::checkRadius(Vector3d & pt)
{
    return radiusSearch( pt );
}

inline int tubeRrtStar::checkNodeUpdate(double new_radius, double old_radius)
{
...
inline bool tubeRrtStar::isSuccessor(NodePtr curPtr, NodePtr nearPtr) // check if curPtr is father of nearPtr
{     
...
inline bool tubeRrtStar::checkValidEnd(NodePtr endPtr)
{
    NodePtr ptr = endPtr;
...
void tubeRrtStar::SafeRegionExpansion( double time_limit )
{   
    // 初始化：记录开始时间用于时间限制判断
    /*  The initialization of the safe region RRT* tree  */
    ros::Time time_bef_expand = ros::Time::now();

    // 新建一个三维 kd-tree（用于存储 RRT 节点以加速邻域查询）
    kdTree_ = kd_create(3);

    // 把当前的起点作为 committed root（提交根），并创建根节点（节点半径由 radiusSearch 计算）
    commit_root  = start_pt;
    root_node = new tubeNode(start_pt, radiusSearch( start_pt ), 0.0, min_distance, 0); 

    // 把根节点记录到 NodeList 中（用于后续管理）
    recordNode(root_node);

    // 将根节点插入 kd-tree（注意 kd-tree 接受 float[3]）
    float pos[3] = {(float)root_node->coord(0), (float)root_node->coord(1), (float)root_node->coord(2)};
    kd_insertf(kdTree_, pos, root_node);
    int iter_count;

    // 主循环：在不超出 max_samples 且未超过 time_limit 的前提下反复采样并扩展树
    for( iter_count = 0; iter_count < max_samples; iter_count ++)
    {     
        // 每次迭代检查是否超时（基于开始时间）
        ros::Time time_in_expand = ros::Time::now();
        if( (time_in_expand - time_bef_expand).toSec() > time_limit ) break;
        
        // 1) 生成采样点（可能包含启发式/目标偏置）
        Vector3d   pt_sample = genSample();
            
        // 2) 在 kd-tree 中找到与采样点最近的已存在节点（最近邻）
        NodePtr node_nearst_ptr = findNearstVertex(pt_sample);

        // 注意：这里存在潜在空指针风险，正确写法应先判断 node_nearst_ptr 是否为 NULL 再访问其成员
        if(node_nearst_ptr == NULL) continue;
        if(!node_nearst_ptr->valid) continue;
                  
        // 3) 使用 tubeSteer 生成一个新的节点；tubeSteer 会尝试沿采样点方向移动并找到与最近节点相交的安全球心
        //    （替代更简单的 genNewNode，tubeSteer 会考虑半径/体积的交集）
        NodePtr node_new_ptr = tubeSteer(pt_sample, node_nearst_ptr);
        
        // 4) 基本可行性检查：高度不能低于地图下界，且节点半径要大于安全裕度
        if( node_new_ptr->coord(2) < z_l  || node_new_ptr->radius < safety_margin ) continue;
           
        // 5) 将新节点接入树并尝试重连附近节点以优化路径（rewire）
        treeRewire(node_new_ptr, node_nearst_ptr);  
        
        // 如果重连后节点被判定为无效，则跳过
        if( ! node_new_ptr->valid) continue;

        // 6) 如果新节点能到达终点，加入 EndList 并更新启发式采样区域
        ...
        // 8) 记录节点到 NodeList，以便后续管理与回溯
        recordNode(node_new_ptr);
        // treePrune(node_new_ptr);      // 可选：按代价裁剪不必要分支

        // 9) 如果无效节点集合达到缓存上限，则清理它们，释放内存并重建 kd-tree
        if( int(invalidSet.size()) >= cach_size) removeInvalid();
    }

    // 循环结束后再次清理无效节点并从 EndList 中追踪最优路径
    removeInvalid();
    tracePath();
...
void tubeRrtStar::SafeRegionRefine( double time_limit )
{   
    ...
    ros::Time time_bef_refine = ros::Time::now();

    float pos[3];
    while( true )
    {     
        ros::Time time_in_refine = ros::Time::now();
        if( (time_in_refine - time_bef_refine).toSec() > time_limit ) break;
...
void tubeRrtStar::SafeRegionEvaluate( double time_limit )
{   
    ...
    {   
        ...
        if(feasibleEndList.size() == 0 || (time_in_evaluate - time_bef_evaluate).toSec() > time_limit ){
    ...
    double repair_limit = time_limit - (time_aft_evaluate - time_bef_evaluate).toSec();
...
void tubeRrtStar::treeRepair(double time_limit, vector< pair<Vector3d, double> > & node_list)
{   
    ...
    ros::Time time_bef_repair = ros::Time::now();
    for(int i = 0; i < repairSize; i++)
    {   
        ros::Time time_in_repair = ros::Time::now();
        if((time_in_repair - time_bef_repair).toSec() > time_limit ) break;
