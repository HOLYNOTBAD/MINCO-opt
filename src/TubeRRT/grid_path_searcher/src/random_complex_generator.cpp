...
bool _has_map  = false, reset_map = false;

//  这里存储生成的随机复杂环境点云，和ros话题发布相关变量
sensor_msgs::PointCloud2 globalMap_pcd;
// pcl的点云格式，储存仅包含点坐标的点云
pcl::PointCloud<pcl::PointXYZ> cloudMap;
// pcl库提供的kdtree，用于加速点云的邻近点搜索，相当于是点云的索引结构
pcl::search::KdTree<pcl::PointXYZ> kdtreeMap;
vector<int>     pointIdxSearch;
vector<float>   pointSquaredDistance;    

void resetMapCallback(const std_msgs::Bool::ConstPtr & msg);
...
{  
   ...
   //生成一个工具 rand_y，以后每次调用它，都能得到一个在 _y_l 到 _y_h 之间随机的小数，而且每个数概率相同。
   ...
   for(int i = 0; i < _cir_num; i ++)
   {
      ...
      // 如果圆心距离初始点太近，则跳过该圆（初始点附近不产生障碍物）
      if(sqrt( pow(x0-_init_x, 2) + pow(y0-_init_y, 2) ) < 2.0 ) 
         continue;


      // Generate ellipse points，生成椭圆的两个缩放系数
      double a, b;
...
{        
   ...
   _reset_map_sub = n.subscribe<std_msgs::Bool>("reset_map", 1,resetMapCallback);
   ...
   n.param("map/reset_map", reset_map, false);
