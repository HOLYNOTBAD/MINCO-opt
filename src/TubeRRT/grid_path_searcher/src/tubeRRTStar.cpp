...
{     
    ...
    // 作用是调用 reset()、setPt()、SafeRegionExpansion(...)
    planInitialPath();
}
...
{   
    ...
    // 用于可视化的点云，点的坐标是四舍五入到栅格中心的
    pcl::PointCloud<pcl::PointXYZ> cloud_vis;
    sensor_msgs::PointCloud2 map_vis;


    //转换与传递：使用 pcl::fromROSMsg 将 sensor_msgs::PointCloud2 
    //转为 pcl::PointCloud<pcl::PointXYZ>，然后将该点云传给路径规划器 _rrtPathPlaner.setInput(cloud)。
    ...
    pcl::PointXYZ pt;
    // 遍历点云，将障碍物信息写入栅格地图，同时生成用于可视化的点云 cloud_vis
    for (int idx = 0; idx < (int)cloud.points.size(); idx++)
    {    
        pt = cloud.points[idx];        

        //遍历点云：对每个点调用 _RRTstar_preparatory->setObs(pt.x, pt.y, pt.z) 将障碍物信息写入栅格/准备器中
        //用于后续路径规划，同时调用 coordRounding 将点坐标四舍五入到网格中心以生成用于可视化的点 cloud_vis。
   ...
   CT = end_time - begin_time;
    ...
    for(int i = 0; i < _Radius.size(); i++)
    {
     ...
     if(repeat_count > repeat_num)
     {
        ...
        ROS_INFO_STREAM("-------------------------[TUBE] plan cost time = " << CT);
        std::cout << "-------------------------[TUBE] mean of radius = " << mean << std::endl;
        std::cout << "-------------------------[TUBE] std var of radius = " <<  stdvar << std::endl;
        repeat_count = 1;
     }

    // cout << "[TUBE]: path found:" << endl;
...
void calcuMetrixs(MatrixXd path, VectorXd radius)
{
    ...
    VectorXd temp(3);
    temp(0) = path.row(path.rows()-1)(0);
    temp(1) = path.row(path.rows()-1)(1);
    temp(2) = path.row(path.rows()-1)(2) ;
    temp = temp - _target_pt;
    ATL = ATL + temp.norm();
    // std::cout << "[TUBE]: ATL = " << ATL << std::endl;

    
    double maxRadius = radius.minCoeff();
    double MSV = 4/3*M_PI*maxRadius*maxRadius*maxRadius;
    // std::cout << "[TUBE]: MSV = " << MSV << std::endl;
    // std::cout << "[TUBE]: CT = " << CT << std::endl;
}
...
/*
    这个节点的作用是：
    1. 接收点云地图，构建栅格地图
    2. 接收目标点，调用 Tube RRT* 算法规划路径
    3. 可视化路径和飞行走廊
*/
int main(int argc, char** argv)
{
    ros::init(argc, argv, "tubeRRTStar");
    ros::NodeHandle nh("~");
    ...
    // 周期调用 ros::spinOnce() 以处理回调函数
    ros::Rate rate(100);
    bool status = ros::ok();

    /*
        主要的几个回调函数有：
        1. rcvPointCloudCallBack：接收点云地图，构建栅格地图
        2. rcvWaypointsCallback：接收目标点，调用 Tube RRT
        3. visRRTstarPath：可视化路径
        4. visFlightCorridor：可视化飞行走廊
        5. planInitialPath：调用 Tube RRT* 算法规划路径
    */
    while(status) 
    {
        ros::spinOnce();      
